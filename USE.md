...
cada um como uma caixa própria (fundo, borda e cantos arredondados) com
ícone, separada das demais por espaçamento — não uma lista de texto corrida:

1. **Dashboard** — caixa inteira é o link, leva para `/admin/` (os cards de
   todos os models registrados).
2. **Framework** — caixa com cabeçalho de seção (não clicável) seguido dos
   models declarados em algum pacote do próprio framework (`kyrux/core/...`),
   como `auth.User`. Útil quando você registra o model de usuário embutido
   para administrar contas pelo painel:
   ```go
   admin.Register[auth.User]("usuarios", "Usuários", admin.SuperuserOnly())
   ```
3. **Aplicações** — caixa com cabeçalho de seção seguido dos models
   declarados nos seus próprios apps (`apps/<nome>/models`).
4. **Logs** — mesmo padrão de Dashboard: a caixa inteira é o link, leva
   direto para `/admin/historico/` (ver
...
(`reflect.Type.PkgPath()`), sem exigir nenhuma Option adicional em
`Register`. Diferente das seções anteriores, os quatro tópicos aparecem
sempre — um tópico sem nenhum model registrado mostra "Nenhum model" em vez
de sumir, mantendo a navegação previsível.
